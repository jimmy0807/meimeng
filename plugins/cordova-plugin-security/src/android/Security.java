...
import com.born.boss.db.SQL.BNDBManager;
import com.born.boss.db.SQL.DBShop;
import com.born.boss.db.UserDefault.PersonalProfile;
import com.born.boss.http.login.LoginFetchUserInfoRequest;
import com.born.boss.ui.member.MemberActivity;
import com.born.boss.ui.member.menu.MemberCardMenuBuyActivity;
import com.born.boss.ui.project.ProjectActivity;
import com.born.tools.Md5Utils;
...
public class Security extends CordovaPlugin {
    ...
    public static final String SERVER_URL = "http://devtop.we-erp.com/MAST/api/app_init";
    ...
    protected boolean launch(CordovaArgs args, final CallbackContext callbackContext) {
        ...
        try {
            ...
            if ( !profile.getUserName().equals(launchMobile) )
            {
                PersonalProfile.currentProfile().delete();
                profile = PersonalProfile.currentProfile();
            }

            if ( !profile.getIsLogin() )
            {
                profile.setBaseUrl(launchUrl);
                profile.setSql(launchDb);
                profile.setUserID(Integer.valueOf(launchUid));
                profile.setUserName(launchMobile);
                profile.setClient_id(Integer.valueOf(launchCid));
                profile.save();

                BNDBManager preManager = BNDBManager.getInstance();
                if ( preManager != null )
                {
                    preManager.close();
                }
                BNDBManager.getInstance(cordova.getActivity(), Md5Utils.MD5(PersonalProfile.currentProfile().getSql() + PersonalProfile.currentProfile().getUserName()) + ".db");
                LoginFetchUserInfoRequest request = new LoginFetchUserInfoRequest();
                request.execute();

                hasFetchUserInfo = true;
            }
            else if ( !hasFetchUserInfo )
            {
                hasFetchUserInfo = true;
            ...
            if ( launchCategory.equals("member") )
            {
                ((MainActivity)cordova.getActivity()).getHandler().postDelayed(new Runnable() {
                    ...
                    public void run() {
                        ...
                        if ( launchSid != null )
                        {
                            DBShop shop = (DBShop)BNDBManager.getInstance().findItem("DBShop", Integer.valueOf(launchSid));
                            if ( shop != null )
                            {
                                MemberActivity.startAct(cordova.getActivity(), shop, MemberActivity.kMemberSelectType);
                            }
                            else
                            {
                                MemberActivity.startAct(cordova.getActivity(), null, MemberActivity.kMemberSelectType);
                            }
                        }
                        else
                        {
            ...
            else if ( launchCategory.equals("product") )
            {
                ((MainActivity)cordova.getActivity()).getHandler().postDelayed(new Runnable() {
                    @Override
                    public void run() {
                        ProjectActivity.startAct(cordova.getActivity(), ProjectActivity.kProjectViewEdit, new ArrayList<String>());
                    }
                }, 0);
            }
            else if ( launchCategory.equals("pos") )
            {
                ((MainActivity)cordova.getActivity()).getHandler().postDelayed(new Runnable() {
                    @Override
                    public void run() {
                        MemberCardMenuBuyActivity.startAct(cordova.getActivity(), MemberCardMenuBuyActivity.kProjectViewBuy);
                    }
                }, 0);
            }
            else if ( launchCategory.equals("guadan") )
            {
                ((MainActivity)cordova.getActivity()).getHandler().postDelayed(new Runnable() {
                    @Override
                    public void run() {
            ...
            else if ( launchCategory.equals("historypos") )
            {
                ((MainActivity)cordova.getActivity()).getHandler().postDelayed(new Runnable() {
                    @Override
                    public void run() {
