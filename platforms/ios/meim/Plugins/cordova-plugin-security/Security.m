...
static NSString* LAUNCH_Browser = @"launch_browser";
...
- (void)launch:(CDVInvokedUrlCommand*)command
{
    [self.commandDelegate runInBackground:^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            else if ([launchCategory isEqualToString:@"browser"])
            {
                NSString* url = params[LAUNCH_Browser];
                CBWebViewController* v = [[CBWebViewController alloc] initWithUrl:url];
...
//                NSDictionary* rdata = params[@"data"];
                NSMutableDictionary *rdata = [NSMutableDictionary dictionaryWithDictionary:params[@"data"]];
                
                _httpUrl = rdata[@"url"];
                _db = rdata[@"db"];
//                songhaibn update 2017年09月28日10:49:20
//                aes_key : byte string
//                The aes_key to use in the symmetric cipher.
//                It must be 16 (*AES-128*), 24 (*AES-192*), or 32 (*AES-256*) bytes long.
                [rdata setObject:@"n:O*@-_91BPqj$zc" forKey:@"aes_key"];
