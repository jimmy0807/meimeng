...
- (void)initWebView
{
    ...
    if ( !self.targetUrl && profile.isLogin )
    {
        ...
        {
            self.targetUrl = [NSString stringWithFormat:@"%@/supplier?born_uuid=%@&phone=%@",SERVER_IP,profile.companyUUID,profile.userName];
...
{
    self.tabBarController.navigationItem.title = @"供应商";
    [self initWebView];
