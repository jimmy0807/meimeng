...
@interface combinationController ()<UITableViewDelegate,UITableViewDataSource,combiEditControllerDelegate>
...
@implementation combinationController
...
{
    //添加组合套完成，刷新界面
    [myNotification addObserver:self selector:@selector(addCompReloadData:) name:packageSeletedCompleted object:nil];
    //保存按钮被点击
    [ myNotification addObserver:self selector:@selector(categoryMainVCSaveBtnClick:) name:productSaveBtnClick object:nil];
}
-(void)addCompReloadData:(NSNotification*)info
{
    if ([info.userInfo[@"tage"] integerValue] != 3) {
        return;
    }
...
-(void)categoryMainVCSaveBtnClick:(NSNotification*)info
{
    ...
    if ([info.userInfo[@"category_id"] integerValue] !=2) {
        return;
    }
    if (!self.baseProjectTemp) {
        if ([self.view isShowingOnKeyWindow]) {
            
            self.parmasDict[@"born_category"] = @3;
        ...
        if (self.deleteArray.count>0) {
            CombiModel *model = [self.deleteArray lastObject];
            [arr addObject:@[@2,model.related.relatedID,@0]];
        }
        parmas[@"pack_line_ids"] = arr;
        parmas[@"born_category"] = @3;
        parmas[@"list_price"] = @(price);
        if ([self.view isShowingOnKeyWindow]) {
            if (self.addArray.count != 0 || self.deleteArray.count != 0) {
...
-(void)setUpDataArray
{
   ...
   NSArray *arr = [projectItem.subRelateds allObjects];
    ...
    for (CDProjectRelated *related in arr) {
        NSLog(@"%@,%@,%@,%@",related.productID,related.parentProductID,related.relatedID,self.baseProjectTemp.templateID);
...
#pragma mark <combiEditControllerDelegate>
-(void)combiEditControllerNumChang:(NSInteger)num andRelated:(CDProjectRelated *)related andTemp:(CDProjectTemplate *)temp
{
    ...
    CDProjectItem *item = [self.baseProjectTemp.projectItems lastObject];
        ...
        if ([[item.subRelateds allObjects] count] ==1) {
            item.subRelateds = [NSSet setWithArray:@[related]];
        }else{
           NSArray *array = [item.subRelateds allObjects];
            NSMutableOrderedSet *orederSet = [NSMutableOrderedSet orderedSet];
            [orederSet addObjectsFromArray:array];
            [orederSet addObject:related];
            item.subRelateds = (NSSet*)orederSet;
        }
        
        self.baseProjectTemp.projectItems = [NSOrderedSet orderedSetWithArray:@[item]];
    [self.tableView reloadData];
    [myNotification postNotificationName:projectSaveBtnHidden object:nil];
}
#pragma mark 添加按钮被点击
- (IBAction)addBtnClick:(UIButton *)sender {
    ...
    ShowCombinationController *showComBination = [[ShowCombinationController alloc]init];
    showComBination.baseProjectTemp = self.baseProjectTemp;
    showComBination.combinTage = 3;
    
    [self.navigationController pushViewController:showComBination animated:YES];
}
@end
