...
class ZixunAllView: UIView , UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout
{
    ...
    {
        self.zixunList = BSCoreDataManager.current().fetchAllZixun(withType: nil, keyword: keyword, memberID: nil, zixunID: nil)
    ...
    func didReceiveNotificationOnMainThread(_ notification : NSNotification)
    {
        ...
        {
            if let searchArray = notification.object as? [CDZixun]
            {
                self.zixunList = searchArray
                self.collectionView.reloadData()
                self.emptyIconImageView.isHidden = self.zixunList.count > 0
            }
            else
    ...
    public func searchBarSearchButtonClicked(_ searchBar: UISearchBar)
    {
        ...
        request.searchKeyWord = searchBar.text
        request.execute()
    }
}
