//
//  HPatientRecipeTempletViewController.m
//  meim
//
...
#import "meim-Swift.h"
#endif
@interface HPatientRecipeTempletViewController ()<HPatientRecipeTempletViewControllerDelegate, UISearchBarDelegate>
...
@implementation HPatientRecipeTempletViewController

- (id)initWithMemberCard:(CDMemberCard *)memberCard couponCard:(CDCouponCard*)couponCard
{
    self = [super initWithNibName:@"HPatientRecipeTempletViewController" bundle:nil];
    if (self)
    {
...
- (void)viewDidLoad
{
    ...
    if ([self.type isEqualToString:@"WenzhenChufang"])
    {
        for ( CDProjectItem* item in self.itemArray )
        {
    ...
    else if ([self.type isEqualToString:@"WenzhenChufangMuban"])
    {
        ChufangMubanRequest *request = [[ChufangMubanRequest alloc] init];
        [request execute];
    }
    else if ([self.type isEqualToString:@"WenzhenJiancha"])
    {
        for ( CDProjectItem* item in self.itemArray )
        {
            if ([item.is_check_service boolValue])
            {
                [showArray addObject:item];
            }
        }
    }
    self.itemArray = [showArray copy];
    ...
    [self registerNofitificationForMainThread:@"ChufangMubanResponse"];
}
...
- (void)didConfirmButtonClick:(id)sender
{
    ...
    if ([self.type isEqualToString:@"WenzhenChufang"])
    {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:@"WenzhenChufangSelectFinished" object:self userInfo:[NSDictionary dictionaryWithObjectsAndKeys:self.posOperate.products.array, @"Chufang", self.data.posOperate, @"POS", nil]];
    ...
    {
        NSLog(@"%@",self.itemArray);
        [[NSNotificationCenter defaultCenter] postNotificationName:@"WenzhenChufangSelectFinished" object:self userInfo:[NSDictionary dictionaryWithObjectsAndKeys:self.posOperate.products.array, @"Jiancha", self.data.posOperate, @"POS", nil]];
    ...
    if ( self.orignalProjectArray )
    {
        [self.delegate didHPatientRecipeTempletViewControllerConfirmButtonPressed:self.posOperate.products.array];
        [self.maskView.navi popViewControllerAnimated:YES];
    }
    else
    {
...
- (void)didMemberCardProjectAdded:(CDProjectItem *)item withUseCount:(NSInteger)count
{
    CDPosProduct* product = [self.data didAddPosOperateWithProjectItem:item withUseCount:count];
    NSNumber* price = self.priceParams[item.itemID];
    if ( price )
    {
        product.product_price = price;
...
- (void)didReceiveNotificationOnMainThread:(NSNotification *)notification
{
    ...
    else if ([notification.name isEqualToString:@"ChufangMubanResponse"])
    {
        if ([[notification.userInfo objectForKey:@"rc"] integerValue] >= 0)
        {
            NSLog(@"%@",notification.userInfo);
            self.itemArray = [notification.userInfo objectForKey:@"data"];
            [self.itemTableView reloadData];
        }
    }
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (tableView == self.itemTableView)
    {
        ...
        if ([self.type isEqualToString:@"WenzhenChufangMuban"]) {
            ...
            cell.textLabel.text = [self.itemArray[indexPath.row] objectForKey:@"name"];
            return cell;
        }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    {
        if ([self.type isEqualToString:@"WenzhenChufangMuban"]) {
            for (NSDictionary *dict in [[self.itemArray objectAtIndex:indexPath.row] objectForKey:@"line"]) {
                CDProjectItem* item = [[BSCoreDataManager currentManager] findEntity:@"CDProjectItem" withValue:[dict objectForKey:@"product_id"] forKey:@"itemID"];
                [self didMemberCardProjectAdded:item withUseCount:[[dict objectForKey:@"qty"] integerValue]];
           }
        }
        else
        {
