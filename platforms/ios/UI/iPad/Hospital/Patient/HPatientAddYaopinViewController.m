//
//  HPatientAddYaopinViewController.m
//  meim
//
...
@interface HPatientAddYaopinViewController ()<HPatientAddYaopinViewControllerDelegate, UISearchBarDelegate>
...
@implementation HPatientAddYaopinViewController

- (id)initWithMemberCard:(CDMemberCard *)memberCard couponCard:(CDCouponCard*)couponCard
{
    self = [super initWithNibName:@"HPatientAddYaopinViewController" bundle:nil];
    if (self)
    {
...
- (void)didConfirmButtonClick:(id)sender
{
    ...
    if ( self.orignalProjectArray )
    {
        [self.delegate didHPatientAddYaopinViewControllerConfirmButtonPressed:self.posOperate.products.array];
        [self.maskView.navi popViewControllerAnimated:YES];
    }
    else
    {
