//
//  ZongkongShoushuMemberTableViewCell.swift
//  meim
//
...
class ZongkongShoushuMemberTableViewCell: UITableViewCell
{
    ...
    var customer: CDZongkongShoushuCustomer?
    {
        didSet
        {
            if let customer = self.customer
            {
                self.avatarImage.sd_setImage(with: URL(string: customer.image_url ?? ""), placeholderImage: UIImage(named: "pad_avatar_default"))
                
                self.memberNameLabel.text = customer.name
                self.doctorNameLabel.text = customer.doctor_name
                
                self.prodLabel.text = customer.product_name ?? ""
                self.designerNameLabel.text = "设计师:" + (customer.designers_name ?? "")
                self.directorNameLabel.text = "设计总监:" + (customer.director_employee_id_name ?? "")
