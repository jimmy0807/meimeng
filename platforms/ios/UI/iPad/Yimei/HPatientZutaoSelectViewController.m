//
//  HPatientZutaoSelectViewController.m
//  meim
//
...
@interface HPatientZutaoSelectViewController ()<HPatientZutaoSelectViewControllerDelegate, UISearchBarDelegate>
...
@implementation HPatientZutaoSelectViewController

- (id)initWithMemberCard:(CDMemberCard *)memberCard couponCard:(CDCouponCard*)couponCard
{
    self = [super initWithNibName:@"HPatientZutaoSelectViewController" bundle:nil];
    if (self)
    {
...
- (void)didConfirmButtonClick:(id)sender
{
    ...
    if ( self.orignalProjectArray )
    {
        [self.delegate didHPatientZutaoSelectViewControllerConfirmButtonPressed:self.posOperate.products.array];
        [self.maskView.navi popViewControllerAnimated:YES];
    }
    else
    {
