...
import { MedicalData, Records } from '../../providers/medical-data';
...
export class MedicalRecords {
  ...
  async getItems(offset = 0, append = false) {
    try {
      let r = await this.medicalData.getMedicalRecords(offset);
      if (r.errcode === 0) {
        let arr = <any[]>r.data;
  ...
  edit(i: Records) {
    this.navCtrl.push("MedicalRecordsDetail", { list: this.list, r: i });
  }

  async delete(i: Records) {
    let r = await this.medicalData.deleteMedicalRecords(i.id);
    this.showToast(r.errmsg);
    if (r.errcode === 0) {
