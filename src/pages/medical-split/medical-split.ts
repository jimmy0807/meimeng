...
import { MedicalSplitDetailPage } from '../medical-split-detail/medical-split-detail';


@Component({
  selector: 'page-medical-split',
  templateUrl: 'medical-split.html'
})
export class MedicalSplitPage {
  s: Split;
  keyword = '';
  list: Split[] = [];
  ...
  async getItems(offset = 0, append = false) {
    try {
      let r = await this.medicalData.getMedicalSplits(offset);
      if (r.errcode === 0) {
        let arr = <any[]>r.data;
  ...
  edit(i: Split) {
    this.navCtrl.push(MedicalSplitDetailPage, { list: this.list, c: i });
  }

  add() {
    this.navCtrl.push(MedicalSplitDetailPage, { list: this.list, c: {} });
  }

  async delete(i: Split) {
    let r = await this.medicalData.deleteMedicalSplit(i.id);
    this.showToast(r.errmsg);
    if (r.errcode === 0) {
