...
@IonicPage()
@Component({
  selector: 'page-medical-advisory-category',
  templateUrl: 'medical-advisory-category.html',
})
export class MedicalAdvisoryCategory {
  t: Item;
  keyword = '';
  list: Item[] = [];
      ...
      },
      {
        ...
        handler: d => {
          if (!d.name)
            return false;
          this.t.name = d.name;
          this.medicalData.saveAdvisoryCategory(this.t).then(
            r => {
              if (r.errcode == 0) {
                if (!this.t.id) {
                  this.t.id = r.data.id;
  ...
  async delete(i: Item) {
    let r = await this.medicalData.deleteAdvisoryCategory(i.id);
    this.showToast(r.errmsg);
    if (r.errcode === 0) {
