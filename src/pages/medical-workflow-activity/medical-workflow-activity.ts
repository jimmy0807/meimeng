...
import { MedicalData, WorkflowActivity } from '../../providers/medical-data';

@IonicPage()
@Component({
  selector: 'page-medical-workflow-activity',
  templateUrl: 'medical-workflow-activity.html',
})
export class MedicalWorkflowActivity {
  keyword = '';
  ...
  async getItems(offset = 0, append = false) {
    try {
      let r = await this.medicalData.getWorkflowActivities(offset);
      if (r.errcode === 0) {
        let arr = <any[]>r.data;
  ...
  edit(i: WorkflowActivity) {
    this.navCtrl.push("MedicalWorkflowActivityDetail", { list: this.list, a: i });
  }

  add() {
    this.navCtrl.push("MedicalWorkflowActivityDetail", { list: this.list, a: {} });
  }

  async delete(i: WorkflowActivity) {
