...
@IonicPage()
@Component({
  selector: 'page-medical-workflow',
  templateUrl: 'medical-workflow.html',
})
export class MedicalWorkflow {
  ...
  async getItems(offset = 0, append = false) {
    try {
      let r = await this.medicalData.getBornWorkflows();
      if (r.errcode === 0) {
        let arr = <any[]>r.data;
  ...
  edit(i: Workflow) {
    this.navCtrl.push("MedicalWorkflowDetail", { list: this.list, w: i });
  }

  add() {
    this.navCtrl.push("MedicalWorkflowDetail", { list: this.list, w: {} });
  }

  async delete(i: Workflow) {
    let r = await this.medicalData.deleteBornWorkflow(i.id);
    this.showToast(r.errmsg);
    if (r.errcode === 0) {
