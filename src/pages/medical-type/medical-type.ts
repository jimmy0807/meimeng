...
@IonicPage()
@Component({
  selector: 'page-medical-type',
  templateUrl: 'medical-type.html',
})
export class MedicalType {
  ...
  async getItems(offset = 0, append = false) {
    try {
      let r = await this.medicalData.getMedicalTypes();
      if (r.errcode === 0) {
        let arr = <any[]>r.data;
      ...
      },
      {
        ...
        handler: d => {
          if (!d.name)
            return false;
          this.t.name = d.name;
          this.medicalData.saveMedicalType(this.t).then(
            r => {
              if (r.errcode == 0) {
                if (!this.t.id) {
                  this.t.id = r.data.id;
  ...
  async delete(i: Item) {
    let r = await this.medicalData.deleteMedicalType(i.id);
    this.showToast(r.errmsg);
    if (r.errcode === 0) {
