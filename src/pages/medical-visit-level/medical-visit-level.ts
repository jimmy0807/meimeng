...
@IonicPage()
@Component({
  selector: 'page-medical-visit-level',
  templateUrl: 'medical-visit-level.html',
})
export class MedicalVisitLevel {
  keyword = '';
  list: VisitLevel[] = [];
  ...
  async getItems(offset = 0, append = false) {
    try {
      let r = await this.medicalData.getVisitLevels();
      if (r.errcode === 0) {
        let arr = <any[]>r.data;
  ...
  edit(i: VisitLevel) {
    this.navCtrl.push("MedicalVisitLevelDetail", { list: this.list, w: i });
  }

  add() {
    this.navCtrl.push("MedicalVisitLevelDetail", { list: this.list, w: {} });
  }

  async delete(i: VisitLevel) {
    let r = await this.medicalData.deleteVisitLevel(i.id);
    this.showToast(r.errmsg);
    if (r.errcode === 0) {
