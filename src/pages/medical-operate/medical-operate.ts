...
@IonicPage()
@Component({
  selector: 'page-medical-operate',
  templateUrl: 'medical-operate.html',
})
export class MedicalOperate {
  keyword = '';
  list: Operate[] = [];
  ...
  async getItems(offset = 0, append = false) {
    try {
      let r = await this.medicalData.getMedicalOperates();
      if (r.errcode === 0) {
        let arr = <any[]>r.data;
  ...
  edit(i: Operate) {
    this.navCtrl.push("MedicalOperateDetail", { list: this.list, w: i });
  }

  add() {
    this.navCtrl.push("MedicalOperateDetail", { list: this.list, w: {} });
  }

  async delete(i: Operate) {
    let r = await this.medicalData.deleteMedicalOperate(i.id);
    this.showToast(r.errmsg);
    if (r.errcode === 0) {
