...
@IonicPage()
@Component({
  selector: 'page-medical-department',
  templateUrl: 'medical-department.html',
})
export class MedicalDepartment {
  ...
  async getItems(offset = 0, append = false) {
    try {
      let r = await this.medicalData.getBornDepartments();
      if (r.errcode === 0) {
        let arr = <any[]>r.data;
  ...
  edit(i: Department) {
    this.navCtrl.push("MedicalDepartmentDetail", { list: this.list, d: i });
  }

  add() {
    this.navCtrl.push("MedicalDepartmentDetail", { list: this.list, d: {} });
  }

  async delete(i: Department) {
    let r = await this.medicalData.deleteBornDepartment(i.id);
    this.showToast(r.errmsg);
    if (r.errcode === 0) {
